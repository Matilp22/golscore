...
import {
  getTeamSelectionFacts,
  type TeamInfoFact,
} from '@/shared/config/team-profile-extra'
...
function TeamCompetitionsCard() {
  return (
    <div className="hf-card w-full overflow-hidden rounded-2xl">
      <div className="hf-section-head px-2 py-2 md:px-3">
        <h2 className="text-base font-bold text-white">Competencias</h2>
      </div>
      <div className="px-2 py-4 text-sm font-semibold text-[#8d98a7] md:px-3">
        No hay competencias sincronizadas para este equipo.
      </div>
    </div>
  )
}
...
export default async function EquipoPage({ params }: PageProps) {
  ...
  const selectionFacts = team.national ? getTeamSelectionFacts(team.name) : null
