...
type SelectionFacts = {
  worldCup: Array<{ label: string; value: string }>
  titles: Array<{ label: string; value: string }>
}

const DEFAULT_SELECTION_FACTS: SelectionFacts = {
  worldCup: [
...
const SELECTION_FACTS_BY_NAME: Record<string, SelectionFacts> = {
  argentina: {
    worldCup: [
...
function getSelectionFacts(name: string) {
  ...
  return (
    <div className="hf-card w-full overflow-hidden rounded-2xl">
      ...
      <div className="px-2 py-1 md:px-3">
        {items.map((item) => (
          <TeamInfoRow key={item.label} label={item.label} value={item.value} />
        ))}
      </div>
    </div>
  )
...
export default async function EquipoPage({ params }: PageProps) {
  ...
  const selectionFacts = team.national ? getSelectionFacts(team.name) : null
      ...
      <div className="w-full max-w-none px-0 py-3 lg:mx-auto lg:max-w-7xl lg:px-5 lg:py-6">
        ...
        <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
          <aside className="space-y-4">
            ...
            {selectionFacts ? (
              <>
                <SelectionFactsCard title="Datos en Mundiales" items={selectionFacts.worldCup} />
                <SelectionFactsCard title="Ranking y titulos" items={selectionFacts.titles} />
              </>
            ) : null}
          </aside>
