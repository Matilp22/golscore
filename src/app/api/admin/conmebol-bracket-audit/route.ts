...
export async function GET(request: Request) {
  ...
  try {
    ...
    const audit = await getConmebolBracketAudit({
      competition,
      leagueExternalId:
    ...
    console.error('[conmebol-bracket-audit] Error completo', error)
    ...
    return jsonNoStore(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'No se pudo auditar bracket Conmebol.',
      },
      { status: 500 }
    )
  }
