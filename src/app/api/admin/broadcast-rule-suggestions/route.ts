...
import { getBroadcastRuleSuggestions } from '@/server/broadcasts/admin'
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await getBroadcastRuleSuggestions(supabase)

    return jsonNoStore({
      endpoint: 'broadcast-rule-suggestions',
      ...result,
    })
  } catch (error) {
    const serialized = serializeError(error, 'unknown')
    console.error('[broadcast-rule-suggestions] Error completo', serialized)
